    ...
    <xsl:template match="/movieRental">
        ...
        <xsl:for-each select="movies/movie">
            ...
            <xsl:variable name="description">
                <xsl:value-of select="substring(description, 0, 100)"></xsl:value-of>
            </xsl:variable>
            ...
            <xsl:for-each select="borrows/borrow">
                ...
                <xsl:variable name="bDay">
                    <xsl:value-of select="substring(borrowDate, 9, 2)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="bMonth">
                    <xsl:value-of select="substring(borrowDate, 6, 2)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="bYear">
                    <xsl:value-of select="substring(borrowDate, 1, 4)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="rDay">
                    <xsl:value-of select="substring(returnDate, 9, 2)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="rMonth">
                    <xsl:value-of select="substring(returnDate, 6, 2)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="rYear">
                    <xsl:value-of select="substring(returnDate, 1, 4)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="borrowDate">
                    <xsl:value-of select="concat($bDay, '/', $bMonth, '/', $bYear)"></xsl:value-of>
                </xsl:variable>
                <xsl:variable name="returnDate">
                    <xsl:value-of select="concat($rDay, '/', $rMonth, '/', $rYear)"></xsl:value-of>
                </xsl:variable>
